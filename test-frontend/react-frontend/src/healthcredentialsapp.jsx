...
export default function HealthCredentialsApp() {
  ...
  const loadCredentials = useCallback(async (address) => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/get/${address}`);
      ...
      if (data.count > 0) {
        showMessage(`✅ Loaded ${data.count} credential(s)`);
      }
    } catch (error) {
...
Return ONLY the JSON, no other text.`
  ...
  const submitAllData = async () => {
    ...
    try {
      ...
      // Merge basic info with extracted data
      const allData = {
        ...
        medical_documents: uploadedFiles.map(f => ({
          file_name: f.name,
      ...
      showMessage(`✅ Submitted! TX: ${result.tx.slice(0, 12)}...`);
      ...
      <section className="content-section">
        <div className="section-card">
          <div className="card-header">
            ...
            <p className="card-subtitle">All your blockchain-verified credentials</p>
          </div>
          ...
          {credentialsLoading ? (
            ...
            <div className="credentials-grid">
              <div className="grid-info">
                <h3>📋 {credentials.length} Submission(s) on Blockchain</h3>
              </div>
                    ...
                    {selectedCredential === index && (
                      <div className="credential-details">
                        <div className="detail-row">
                          <span className="detail-label">Your Health Data:</span>
                          <pre className="detail-value">{JSON.stringify(cred, null, 2)}</pre>
                        </div>
                      </div>
                    )}
                  </div>
      ...
      <section className="content-section">
        <div className="section-card">
          <div className="card-header">
            ...
            <p className="card-subtitle">Healthcare providers can verify your credentials</p>
          </div>
              ...
              {credentials.map((cred, index) => (
                <div key={index} className="verify-card">
                  ...
                  <div className="verify-actions">
                    <button 
                      onClick={() => verifyCredential(index, true)}
                    ...
                    <button 
                      onClick={() => verifyCredential(index, false)}
                      className="btn btn-danger"
                      disabled={loading}
                    >
                      <AlertCircle size={16} />
